...
public class PackageServiceImpl implements PackageService {
	...
	public List<Package> getAllFilteredPackages(String activity, String interest, String destination, String maxPrice) {
		...
		if (destination.equals("null")) {
			...
			packages = packages.stream().distinct().filter(package1 -> package1.getCityName() == destination)
		...
		if (maxPrice.equals("null")) {
			...
			packages = packages.stream().distinct().filter(package1 -> package1.getPrice() <= Double.valueOf(maxPrice))
					.filter(package1 -> package1.getCityName() == destination).collect(Collectors.toList());
		}
	...
	public PackageDto updatePackage(PackageDto packageDto) {
		...
		isPackageExist.setType(packageDto.getType());
		isPackageExist.setActivities(packageDto.getActivities());
		isPackageExist.setAreaOfInterest(packageDto.getAreaOfInterest());
